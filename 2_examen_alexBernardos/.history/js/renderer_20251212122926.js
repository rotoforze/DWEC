...
function addPlan(horario) {
    if (!planes.includes(horario)) {
        ...
        const id = planes.push(horario)
        document.querySelector("#plan-list").innerHTML += (`
            <li class="plan-item-info" data-id="${planes.length -1}">
                ...
                <div class="plan-item-actions">
                    ...
                    <button class="btn-delete" onclick="${() => {document.querySelector(`[data-id="${planes.length -1}"]`)}}" data-planid="${planes.length -1}">Eliminar</button>
                </div>
            </li>
            `);
    }
