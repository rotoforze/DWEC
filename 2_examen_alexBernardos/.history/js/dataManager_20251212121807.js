...
/**
 * Carga los datos de los JSON, añade el contenido y pone los eventos.
 * Básicamente carga todo
 */
...
/**
 * Recibe un id de artista, un id de escenario y una fecha
 * y devuelve el objeto que coinicida con esas especificaciones
 * 
 ...
 * @returns {Object}
 */
export function getHorario(artistaId, escenarioId, dia) {
        for (const horario of horarios) {
        if (horario.artistaId == artistaId && horario.escenarioId == escenarioId 
            && horario.dia == dia) {
            return horario;
        }
    }
...
function filtrar() {
    ...
    if (busqueda) {
        ...
        horariosFiltrados = horariosFiltrados.filter((horario) => {
            console.log(getArtista(horario.artistaId).nombre.toLowerCase())
            if (getArtista(horario.artistaId).nombre.toLowerCase().includes(busqueda.toLowerCase())) {
                return horario;
            }
        })
    }
